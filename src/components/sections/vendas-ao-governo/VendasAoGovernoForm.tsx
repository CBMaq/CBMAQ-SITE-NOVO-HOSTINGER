...
export function VendasAoGovernoForm() {
  ...
  const onSubmit = async (data: FormData) => {
    const lastSubmission = localStorage.getItem('cbmaq_vendasgov_submission');
    if (lastSubmission) {
    ...
    try {
      const response = await fetch("/api/vendas-ao-governo", {
        method: "POST",
      ...
      if (response.ok) {
        setStatus("success");
        setIsModalOpen(true);
        localStorage.setItem('cbmaq_vendasgov_submission', Date.now().toString());
        reset();
      } else {
        setStatus("error");
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      ...
                      <option value="Licitação/Pregão">Licitação/Pregão</option>
