...
const formSchema = z.object({
  ...
  manufactureYear: z.string().min(4, "Ano de fabricação é obrigatório"),
  serialNumber: z.string().min(1, "Série da máquina é obrigatória"),
  approximateValue: z.string().min(1, "Valor Aproximado do Equipamento é obrigatório"),
  message: z.string().optional().or(z.literal("")),
});

type FormData = z.infer<typeof formSchema>;

export function TelemetriaFormSection() {
  ...
  const onSubmit = async (data: FormData) => {
    const lastSubmission = localStorage.getItem('cbmaq_telemetria_submission');
    if (lastSubmission) {
    ...
    try {
      ...
      if (response.ok) {
        setStatus("success");
        setIsModalOpen(true);
        localStorage.setItem('cbmaq_telemetria_submission', Date.now().toString());
        reset();
      } else {
        setStatus("error");
          ...
          <div className="max-w-[1100px] mx-auto">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
                ...
                {/* Bloco Esquerda (Tipo, Ano, Valor) - Ocupa 2 colunas */}
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                  <div className="flex flex-col">
                    <label className="text-[1rem] font-bold text-[#0A2A5E] mb-2">Tipo de Equipamento</label>
                    <input
                      {...register("equipmentType")}
                      className="h-[3.5rem] px-5 rounded-[12px] bg-[#F8F9FA] border-[#E9ECEF] border-[1px] focus:border-[#0A4EE4] outline-none transition-all font-medium text-[#001647]"
                    />
                    <div className="h-4 mt-1">
                      {errors.equipmentType && <span className="text-red-500 text-[10px] font-bold block">{errors.equipmentType.message}</span>}
                    </div>
                  </div>

                  <div className="flex flex-col">
                    <label className="text-[1rem] font-bold text-[#0A2A5E] mb-2">Marca</label>
                    <input
                      {...register("brand")}
                      className="h-[3.5rem] px-5 rounded-[12px] bg-[#F8F9FA] border-[#E9ECEF] border-[1px] focus:border-[#0A4EE4] outline-none transition-all font-medium text-[#001647]"
                    />
                    <div className="h-4 mt-1">
                      {errors.brand && <span className="text-red-500 text-[10px] font-bold block">{errors.brand.message}</span>}
                    </div>
                  </div>

                  <div className="flex flex-col">
                    <label className="text-[1rem] font-bold text-[#0A2A5E] mb-2">Ano de Fabricação</label>
                    <input
                      {...register("manufactureYear")}
                      className="h-[3.5rem] px-5 rounded-[12px] bg-[#F8F9FA] border-[#E9ECEF] border-[1px] focus:border-[#0A4EE4] outline-none transition-all font-medium text-[#001647]"
                    />
                    <div className="h-4 mt-1">
                      {errors.manufactureYear && <span className="text-red-500 text-[10px] font-bold block">{errors.manufactureYear.message}</span>}
                    </div>
                  </div>

                  <div className="flex flex-col">
                    <label className="text-[1rem] font-bold text-[#0A2A5E] mb-2">Série da Máquina</label>
                    <input
                      {...register("serialNumber")}
                      className="h-[3.5rem] px-5 rounded-[12px] bg-[#F8F9FA] border-[#E9ECEF] border-[1px] focus:border-[#0A4EE4] outline-none transition-all font-medium text-[#001647]"
                    />
                    <div className="h-4 mt-1">
                      {errors.serialNumber && <span className="text-red-500 text-[10px] font-bold block">{errors.serialNumber.message}</span>}
                    </div>
                  </div>

                  <div className="md:col-span-2 flex flex-col">
                    <label className="text-[1rem] font-bold text-[#0A2A5E] mb-2">Valor Aproximado do Equipamento</label>
                    <input
                    ...
                    />
                    <div className="h-4 mt-1">
                      {errors.approximateValue && <span className="text-red-500 text-[10px] font-bold block">{errors.approximateValue.message}</span>}
                    </div>
                  </div>
                </div>
