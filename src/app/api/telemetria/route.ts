...
import { getTelemetriaEmailHtml } from "@/components/emails/TelemetriaEmailTemplate";
...
export async function POST(req: Request) {
  try {
    ...
    const emailHtml = getTelemetriaEmailHtml(validatedData);

    const { data, error } = await resend.emails.send({
      from: `CBMaq Site <${fromEmail}>`,
      to: [toEmail],
      subject: `Nova solicitação - Telemetria (${validatedData.fullName})`,
      html: emailHtml,
    });
