...
import { getRevendedorEmailHtml } from "@/components/emails/RevendedorEmailTemplate";
...
export async function POST(req: Request) {
  try {
    ...
    const emailHtml = getRevendedorEmailHtml(validatedData);

    const { data, error } = await resend.emails.send({
      from: `CBMaq Site <${fromEmail}>`,
      to: [toEmail],
      subject: `Novo cadastro de revendedor - CBMaq (${validatedData.fullName})`,
      html: emailHtml,
    });
