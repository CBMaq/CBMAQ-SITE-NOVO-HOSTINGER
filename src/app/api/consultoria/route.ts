...
import { getConsultoriaEmailHtml } from "@/components/emails/ConsultoriaEmailTemplate";
...
const consultoriaSchema = z.object({
  ...
  maquinarios: z.string().min(2, "Maquinários atuais são obrigatórios"),
...
export async function POST(req: Request) {
  try {
    ...
    const validatedData = consultoriaSchema.parse(body);
    ...
    const emailHtml = getConsultoriaEmailHtml(validatedData);

    const { data, error } = await resend.emails.send({
      from: `CBMaq Consultoria <${fromEmail}>`,
      to: [toEmail],
      subject: `Novo Lead de Consultoria: ${validatedData.fullName} (${validatedData.area})`,
      html: emailHtml,
    });
    ...
    return NextResponse.json({ 
      success: true, 
      message: "Consultoria solicitada com sucesso!" 
    });
