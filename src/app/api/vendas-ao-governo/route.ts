...
import { getVendasAoGovernoEmailHtml } from "@/components/emails/VendasAoGovernoEmailTemplate";
...
export async function POST(req: Request) {
  try {
    ...
    const emailHtml = getVendasAoGovernoEmailHtml(validatedData);

    const { data, error } = await resend.emails.send({
      from: `CBMaq Site <${fromEmail}>`,
      to: [toEmail],
      subject: `Nova solicitação - Vendas ao Governo (${validatedData.fullName})`,
      html: emailHtml,
    });
